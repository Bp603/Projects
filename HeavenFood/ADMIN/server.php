...
include_once 'Config.php';
	...
	if (isset($_POST['reg_user']))
	{
		...
		if (!preg_match("/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}*$/", $password_2)) 
		{
	...
	$user_check_query = "SELECT * FROM COOKREG WHERE USERNAME ='$username' OR EMAIL='$email' LIMIT 1";
	...
	if (isset($_POST['btnlogin'])) {
		$email = mysqli_real_escape_string($lk, $_POST['uname']);
		$password = mysqli_real_escape_string($lk, $_POST['pwd']);

		// empty file email and password
		if (empty($email)) {
			array_push($errors, '<div class="alert-danger" role="alert">username is required</div>');
		}

        if (!preg_match("/^[a-zA-Z]*$/", $email)) 
		{
			array_push($errors, '<div class="alert-danger" role="alert">username is only contain alphabets.</div>');
		}
		...
		// {
		// 	array_push($errors, '<div class="alert-danger" role="alert">Password is only contain number.</div>');
		// }


		if (count($errors) == 0) {
			$password = ($password);
			$query = "SELECT * FROM ADMIN WHERE UNAME='$email' and PASSWD='$password'";
