...
include_once 'Config.php';
	...
	if (isset($_POST['reg_user']))
	{
		...
		$usr_chk_qry = "SELECT * FROM WAITER WHERE WFNAME ='$username' LIMIT 1";
	...
	$user_check_query = "SELECT * FROM WAITERREG WHERE USERNAME ='$username' OR EMAIL='$email' LIMIT 1";
	...
	if (count($errors) == 0)
		{
			...
			$query = "INSERT INTO `WAITERREG`(`USERNAME`,`EMAIL`, `PHONE`, `PASSWD`)
	...
	if (isset($_POST['login_user'])) {
		...
		if (count($errors) == 0) {
			...
			$query = "SELECT * FROM WAITERREG WHERE EMAIL='$email' and PASSWD='$password'";
